...
class ContextualGoalWithVasesLevel1(BaseTask):
    """
    ...
    def __init__(
        ...
        self.placements_conf.extents = np.array([-1.2, -1.5, -1.2, -1.5])

        self.vase_size = 0.1
        self.vase_placement_bottom = np.array([-0.25, -2.25, 0.25, -1.85])
        self.vase_placement_right = np.array([0.75, -0.5, 1.05, 0.0])
        self.vase_placement_top = np.array([-0.25, 0.5, 0.25, 0.9])
        self.vase_placement_bottom_right = np.array([1.25, -2.25, 1.75, -1.75])

        self._add_free_geoms(Vases(size=self.vase_size, keepout=0.15, num=3, is_constrained=True,
                                   placements=[self.vase_placement_bottom,
                                               self.vase_placement_right,
                                               self.vase_placement_top,
                                               self.vase_placement_bottom_right,
                                               ]))
...
tasks = {
    "ContextualGoalWithVasesLevel1": ContextualGoalWithVasesLevel1
}
