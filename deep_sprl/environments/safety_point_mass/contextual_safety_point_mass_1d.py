...
@env_register
class ContextualSafetyPointMass1D(CMDP):
    _support_envs: ClassVar[list[str]] = ['ContextualSafetyPointMass1D-v0']
    ...
    def reset(
        ...
        if seed is not None:
            self.set_seed(seed)
        self._state = torch.as_tensor([-self.ROOM_WIDTH/2+0.5, 0., 3, 0.])
        return self._state, {}
    ...
    def _step_internal(self, state, action):
        ...
        ###### EXAMPLE ########
        on_lava = False
        if (new_state[2] >= -1.0 and new_state[2] <= 1.0) and (
