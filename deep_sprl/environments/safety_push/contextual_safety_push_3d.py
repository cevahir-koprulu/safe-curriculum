...
@env_register
class ContextualSafetyPush3D(CMDP):
    _support_envs: ClassVar[list[str]] = ['ContextualSafetyPush3D-v0']
    ...
    def __init__(self, 
        ...
        self.pos_threshold = self.CYLINDER_RADIUS # 0.05 # 0.02
        ...
        self._env = robosuite.make(
            ...
            cylinder_xrange=(-0.18,-0.18),#(-0.25,-0.25),# (-0.275,-0.275), #(-0.3, -0.3),
            cylinder_yrange=(0.0,0.0),#(-0.05, 0.05),
        )
    ...
    def step(self, action):
        ...
        if self.sparse_reward:
            ...
            # DENSE-4: change of distance bt puck & eef
            is_eff_far = self.dist_puck_eff > (self.CYLINDER_RADIUS*self.eef_dist_th)
            reward = torch.as_tensor((self.dist_goal_puck-new_dist_goal_puck)+(
                self.dist_puck_eff-new_dist_puck_eff)*is_eff_far + self.goal_rew_coef * success)
