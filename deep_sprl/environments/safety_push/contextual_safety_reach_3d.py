...
@env_register
class ContextualSafetyReach3D(CMDP):
    _support_envs: ClassVar[list[str]] = ['ContextualSafetyReach3D-v0']
    metadata: ClassVar[dict[str, int]] = {'render_fps': 10}
    need_auto_reset_wrapper = True
    need_time_limit_wrapper = True
    _num_envs = 1

    MAX_TIME_STEPS = 100 # 500
    ...
    def __init__(self, 
        ...
        self._context = np.array([0.875, 0.5, 1.0]) # gx, gy, hy
        ...
        self._env = robosuite.make(
            ...
            cylinder_xrange=(0.5,0.5), # puck is outside the table
            cylinder_yrange=(0.0,0.0),
        )
        self._observation_space = spaces.Box(-np.inf, np.inf, (2,), dtype=np.float32)
    ...
    def reset(
        ...
        self.dist_goal_eff = np.linalg.norm(obs["robot0_eef_pos"][:2] - self._real_context[:2])
        print(f"Initial state: {self._state} || Context: {self._context} || Real context: {self._real_context}")
    ...
    def step(self, action):
        ...
        new_dist_goal_eff = np.linalg.norm(obs["robot0_eef_pos"][:2] - self._real_context[:2])
        self.obs_robotsuite = obs
        self._state = torch.as_tensor(
            np.concatenate((
                # obs["object_pos"][:2],
        ...
        success = torch.as_tensor(np.linalg.norm(obs["robot0_eef_pos"][:2] - self._real_context[:2]) < self.pos_threshold)    
        if self.sparse_reward:
            # reward = torch.as_tensor(-1.0 * (not success))
            reward = torch.as_tensor(1.0 * success)
        else:
            reward = torch.as_tensor((self.dist_goal_eff-new_dist_goal_eff) + 1.0 * success)

        # self.dist_goal_puck = new_dist_goal_puck
        # self.dist_puck_eff = new_dist_puck_eff
        self.dist_goal_eff = new_dist_goal_eff
        in_hazard = (self.hazard_x_range[0]-self.safety_radius <= self._state[0] <= self.hazard_x_range[1]+self.safety_radius) and (
            -self._real_context[2]-self.safety_radius <= self._state[1] <= self._real_context[2]+self.safety_radius)
        
        # if in_hazard:
        #     print(f"state: {self._state} || context: {self._real_context}")

        cost = torch.as_tensor(self._single_pass_cost * in_hazard)
