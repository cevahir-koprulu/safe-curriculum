...
from deep_sprl.environments.safety_ant import ContextualSafetyAnt3D
...
class GoalSampler:
    ...
    def sample(self):
        ...
        while not ContextualSafetyAnt3D._is_feasible(sample):
            sample = np.random.uniform(self.LOWER_CONTEXT_BOUNDS, self.UPPER_CONTEXT_BOUNDS)
        return sample
...
class SafetyAnt3DExperiment(AbstractExperiment):
    ...
    NUM_ITER = 100 # 150
    STEPS_PER_ITER = 2500 # 10000
    STEPS_PER_EPOCH = 2500 # 10000
    ...
    def create_environment(self, evaluation=False):
        env_id = "ContextualSafetyAnt3D-v0"
        special_kwargs = {}
    ...
    def create_learner_params(self):
        algo_specific_cfgs = {
            ...
            Learner.PPOLag:  {
                'algo_cfgs': {
                    'steps_per_epoch': self.STEPS_PER_EPOCH, # to eval, log, actor scheduler step
                    'update_iters': 10, # 15, # 20, # gradient steps
    ...
    def create_experiment(self):
        ...
        if str(model.agent._env._env.teacher)=="vds":
            ...
            state_provider = lambda contexts: np.concatenate(
                (ContextualSafetyAnt3D.sample_initial_state(contexts.shape[0]), contexts.shape), axis=-1),
