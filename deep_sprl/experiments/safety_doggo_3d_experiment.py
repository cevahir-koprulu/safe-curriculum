...
from deep_sprl.environments.safety_doggo import ContextualSafetyDoggo3D

class GoalSampler:
    def __init__(self):
        # self.LOWER_CONTEXT_BOUNDS = np.array([-1.5, 1., 0.25])
        # self.UPPER_CONTEXT_BOUNDS = np.array([1.5, 1.5, 0.5])
        # self.LOWER_CONTEXT_BOUNDS = np.array([-1., 0.5, 0.2])
        # self.UPPER_CONTEXT_BOUNDS = np.array([1., 1., 0.25])
        self.LOWER_CONTEXT_BOUNDS = np.array([-0.5, 0.25, 0.1])
        self.UPPER_CONTEXT_BOUNDS = np.array([0.5, 0.5, 0.125])

    def sample(self):
        sample = np.random.uniform(self.LOWER_CONTEXT_BOUNDS, self.UPPER_CONTEXT_BOUNDS)
        while not ContextualSafetyDoggo3D._is_feasible(sample):
            sample = np.random.uniform(self.LOWER_CONTEXT_BOUNDS, self.UPPER_CONTEXT_BOUNDS)
        return sample
...
class SafetyDoggo3DExperiment(AbstractExperiment):
    ...
    # INITIAL_MEAN = np.array([-0., 0., 0.35])
    # INITIAL_VARIANCE = np.diag(np.square([0.5, 0.5, 0.15]))
    INITIAL_MEAN = np.array([-0., 0., 0.175])
    INITIAL_VARIANCE = np.diag(np.square([0.25, 0.25, 0.025]))
    ...
    # TARGET_TOLERANCE_W = 0.05
    # TARGET_LOWER_CONTEXT_BOUNDS = np.array([-1., 0.5, TARGET_TOLERANCE])
    # TARGET_UPPER_CONTEXT_BOUNDS = np.array([1., 1., TARGET_TOLERANCE+TARGET_TOLERANCE_W])
    TARGET_TOLERANCE = 0.1
    TARGET_TOLERANCE_W = 0.025
    TARGET_LOWER_CONTEXT_BOUNDS = np.array([-0.5, 0.25, TARGET_TOLERANCE])
    TARGET_UPPER_CONTEXT_BOUNDS = np.array([0.5, 0.5, TARGET_TOLERANCE+TARGET_TOLERANCE_W])

    # LOWER_CONTEXT_BOUNDS = np.array([-1.5, -1.5, TARGET_TOLERANCE])
    # UPPER_CONTEXT_BOUNDS = np.array([1.5, 1.5, 0.75])
    # LOWER_CONTEXT_BOUNDS = np.array([-1., -1., TARGET_TOLERANCE])
    # UPPER_CONTEXT_BOUNDS = np.array([1., 1., 0.5])
    LOWER_CONTEXT_BOUNDS = np.array([-0.5, -0.5, TARGET_TOLERANCE])
    UPPER_CONTEXT_BOUNDS = np.array([0.5, 0.5, 0.25])
    ...
    EP_PER_UPDATE = 30 # 120 # 40
    PEN_COEFT = 0.0
    ATP = 0.75 # 1 # annealing target probability for CCURROT
    ...
    NUM_ITER = 100 # 150
    STEPS_PER_ITER = 20000 # 2500 # 10000
    STEPS_PER_EPOCH = 20000 # 2500 # 10000
    ...
    def create_environment(self, evaluation=False):
        env_id = "ContextualSafetyDoggo3D-v0"
        special_kwargs = {}
    ...
    def create_learner_params(self):
        algo_specific_cfgs = {
            ...
            Learner.PPOLag:  {
                'algo_cfgs': {
                    'steps_per_epoch': self.STEPS_PER_EPOCH, # to eval, log, actor scheduler step
                    'update_iters': 40, # 10, # 15, # 20, # gradient steps
            ...
            Learner.CPO:  {
                'algo_cfgs': {
                    'steps_per_epoch': self.STEPS_PER_EPOCH, # to eval, log, actor scheduler step
                    'update_iters': 40, # 20, # 15, # gradient steps
    ...
    def create_experiment(self):
        ...
        if str(model.agent._env._env.teacher)=="vds":
            ...
            state_provider = lambda contexts: np.concatenate(
                (ContextualSafetyDoggo3D.sample_initial_state(contexts.shape[0]), contexts.shape), axis=-1),
    ...
    def create_self_paced_teacher(self):
        ...
        # init_lb = np.concatenate((np.array([-1.,-1.]),[self.UPPER_CONTEXT_BOUNDS[-1]]))
        # init_ub = np.concatenate((np.array([1.,-0.5]),[self.UPPER_CONTEXT_BOUNDS[-1]]))
        init_lb = np.concatenate((np.array([-0.5,-0.5]),[self.UPPER_CONTEXT_BOUNDS[-1]]))
        init_ub = np.concatenate((np.array([0.5,-0.25]),[self.UPPER_CONTEXT_BOUNDS[-1]]))
