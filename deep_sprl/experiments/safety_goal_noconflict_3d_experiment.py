...
from deep_sprl.environments.safety_goal import ContextualSafetyGoalNoConflict3D
...
class GoalSampler:
    ...
    def sample(self):
        ...
        while not ContextualSafetyGoalNoConflict3D._is_feasible(sample):
            sample = np.random.uniform(self.LOWER_CONTEXT_BOUNDS, self.UPPER_CONTEXT_BOUNDS)
        return sample
...
class SafetyGoalNoConflict3DExperiment(AbstractExperiment):
    ...
    def create_environment(self, evaluation=False):
        env_id = "ContextualSafetyGoalNoConflict3D-v0"
        special_kwargs = {}
    ...
    def create_experiment(self):
        ...
        if str(model.agent._env._env.teacher)=="vds":
            ...
            state_provider = lambda contexts: np.concatenate(
                (ContextualSafetyGoalNoConflict3D.sample_initial_state(contexts.shape[0]), contexts.shape), axis=-1),
    ...
    def get_env_name(self):
        return f"safety_goal_noconflict_3d"
