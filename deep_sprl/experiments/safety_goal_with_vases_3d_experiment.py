...
from deep_sprl.environments.safety_goal_with_vases import ContextualSafetyGoalWithVases3D

class GoalWithVasesSampler:
    def __init__(self):
    ...
    def sample(self):
        ...
        while not ContextualSafetyGoalWithVases3D._is_feasible(sample):
            sample = np.random.uniform(self.LOWER_CONTEXT_BOUNDS, self.UPPER_CONTEXT_BOUNDS)
        return sample
...
class SafetyGoalWithVases3DExperiment(AbstractExperiment):
    ...
    def create_environment(self, evaluation=False):
        env_id = "ContextualSafetyGoalWithVases3D-v0"
        special_kwargs = {}
        if evaluation or self.curriculum.default():
            teacher = GoalWithVasesSampler()
    ...
    def create_experiment(self):
        ...
        if str(model.agent._env._env.teacher)=="vds":
            ...
            state_provider = lambda contexts: np.concatenate(
                (ContextualSafetyGoalWithVases3D.sample_initial_state(contexts.shape[0]), contexts.shape), axis=-1),
    ...
    def get_env_name(self):
        return f"safety_goal_with_vases_3d"
