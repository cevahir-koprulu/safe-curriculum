...
class SafetyPointMass2D2Experiment(AbstractExperiment):
    TARGET_TYPE = "narrow"
    ...
    def create_environment(self, evaluation=False):
        env = gym.make("ContextualSafetyPointMass2D2-v1")
    ...
    def get_env_name(self):
        return f"safety_point_mass_2d_2_{self.TARGET_TYPE}"
    ...
    def evaluate_learner(self, path, eval_type=""):
        ...
        for i in range(num_context):
            ...
            for j in range(num_run):
                ...
                all_costs[i] += np.cumprod((np.ones(400)*self.DISCOUNT_FACTOR))/self.DISCOUNT_FACTOR@np.array(costs) / num_run
                all_returns[i] += np.cumprod((np.ones(400)*self.DISCOUNT_FACTOR))/self.DISCOUNT_FACTOR@np.array(returns) / num_run
