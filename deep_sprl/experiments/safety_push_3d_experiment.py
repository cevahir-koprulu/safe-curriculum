...
from deep_sprl.environments.safety_push import ContextualSafetyPush3D
...
class PushSampler:
    ...
    def sample(self):
        ...
        while not ContextualSafetyPush3D._is_feasible(sample):
            sample = np.random.uniform(self.TARGET_LOWER_CONTEXT_BOUNDS, self.TARGET_UPPER_CONTEXT_BOUNDS)
        return sample
...
class SafetyPush3DExperiment(AbstractExperiment):
    ...
    INITIAL_MEAN = np.array([0.0, 0.125, 0.5])
    INITIAL_VARIANCE = np.diag(np.square([0.25, 0.125, 0.25]))
    TARGET_HY = 0.9
    TARGET_HY_W = 0.1
    TARGET_LOWER_CONTEXT_BOUNDS = np.array([0.75, 0.0, TARGET_HY])
    TARGET_UPPER_CONTEXT_BOUNDS = np.array([1.0,  1.0, TARGET_HY+TARGET_HY_W])
    LOWER_CONTEXT_BOUNDS = np.zeros(3)
    UPPER_CONTEXT_BOUNDS = np.ones(3)
    ...
    def target_log_likelihood(self, cs):
        ...
        norm = (area_target * 1 + area_rest * min_p) * (
            self.TARGET_HY_W  + ((u2 - l2)-(self.TARGET_HY_W)) * min_p) 
    ...
    def target_sampler(self, n, rng=None):
        if rng is None:
            rng = np.random
        return rng.uniform(self.TARGET_LOWER_CONTEXT_BOUNDS, 
                           self.TARGET_UPPER_CONTEXT_BOUNDS, 
                           size=(n, self.TARGET_LOWER_CONTEXT_BOUNDS.shape[0]))

    DIST_TYPE = "gaussian"  # "cauchy"

    STD_LOWER_BOUND = np.array([0.05, 0.05, 0.05])
    KL_THRESHOLD = 8000.
    KL_EPS = 0.1 # 0.25
    DELTA = 0.6
    DELTA_CS = 0.0 
    DELTA_CT = 1.0
    METRIC_EPS = 0.1 # 0.05 # 0.25 # 0.5
    INIT_CONTEXT_NUM = 100 # 200
    EP_PER_UPDATE = 20 # 40 # 10
    PEN_COEFT = 0.0
    ...
    NUM_ITER = 150 # 200 # 300
    STEPS_PER_ITER = 10000
    STEPS_PER_EPOCH = 10000 # 4000
    ...
    def create_environment(self, evaluation=False):
        env_id = "ContextualSafetyPush3D-v0"
        special_kwargs = {}
        if evaluation or self.curriculum.default():
            ...
            bins = 10
            teacher = ACL(bins**self.LOWER_CONTEXT_BOUNDS.shape[0], self.ACL_ETA, eps=self.ACL_EPS, norm_hist_len=2000)
            teacher_id = "ACL"
            special_kwargs['context_post_processing'] = Subsampler(self.LOWER_CONTEXT_BOUNDS.copy(),
                                                                     self.UPPER_CONTEXT_BOUNDS.copy(),
                                                                     [bins for _ in range(self.LOWER_CONTEXT_BOUNDS.shape[0])])
    ...
    def create_learner_params(self):
        algo_specific_cfgs = {
            ...
            Learner.PPOLag:  {
                'algo_cfgs': {
                    'steps_per_epoch': self.STEPS_PER_EPOCH, # to eval, log, actor scheduler step
                    'update_iters': 20, # 15, # 12, # 25, # 40, # 12 # gradient steps
            ...
            Learner.CPO:  {
                'algo_cfgs': {
                    'steps_per_epoch': self.STEPS_PER_EPOCH, # to eval, log, actor scheduler step
                    'update_iters': 15, # 20, # 15, # gradient steps
        ...
        custom_cfgs = {
            ...
            'model_cfgs':  {
                ...
                'actor': {
                    'hidden_sizes': [128, 128], # [256, 256], # [256, 256, 256], # [64, 64],
                    'activation': "tanh", 
                    'lr': 3e-4,
                },
                'critic': {
    ...
    def create_experiment(self):
        ...
        if str(model.agent._env._env.teacher)=="vds":
            ...
            state_provider = lambda contexts: np.concatenate(
                (ContextualSafetyPush3D.sample_initial_state(contexts.shape[0]), contexts.shape), axis=-1),
    ...
    def create_self_paced_teacher(self):
        ...
        ###############
        #### HELLO ####
        init_lb = self.LOWER_CONTEXT_BOUNDS.copy()
        init_ub = np.concatenate((np.array([0.25]), self.UPPER_CONTEXT_BOUNDS[1:2], np.array([0.1])))
        ###############
        ###############
