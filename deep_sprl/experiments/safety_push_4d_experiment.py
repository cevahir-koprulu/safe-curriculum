...
from deep_sprl.environments.safety_push import ContextualSafetyPush4D

class PushSampler:
    def __init__(self):
        self.TARGET_HY = 0.0
        self.TARGET_HY_W = 0.1
        self.TARGET_TOLERANCE = 0.0
        self.TARGET_TOLERANCE_W = 0.1
        self.TARGET_LOWER_CONTEXT_BOUNDS = np.array([0.75, 0.0, 
                                                     self.TARGET_HY, self.TARGET_TOLERANCE])
        self.TARGET_UPPER_CONTEXT_BOUNDS = np.array([1.0,  1.0, 
                                                     self.TARGET_HY+self.TARGET_HY_W, 
                                                     self.TARGET_TOLERANCE+self.TARGET_TOLERANCE_W])

    def sample(self):
        sample = np.random.uniform(self.TARGET_LOWER_CONTEXT_BOUNDS, self.TARGET_UPPER_CONTEXT_BOUNDS)
        while not ContextualSafetyPush4D._is_feasible(sample):
            sample = np.random.uniform(self.TARGET_LOWER_CONTEXT_BOUNDS, self.TARGET_UPPER_CONTEXT_BOUNDS)
        return sample
...
class SafetyPush4DExperiment(AbstractExperiment):
    PEN_COEFS = {Learner.SAC: 0.0, 
                Learner.PPO: 1.0, 
                Learner.PPOLag: 0.0}

    INIT_VAR = 0.0 # Redundant
    # INITIAL_MEAN = np.array([-0.3, 0.0, -0.075, 0.075, 0.125])
    # INITIAL_VARIANCE = np.diag(np.square([0.025, 0.1, 0.025, 0.025, 0.025]))
    # TARGET_H_W = 0.05
    # TARGET_TOLERANCE = 0.05
    # TARGET_TOLERANCE_W = 0.025
    # TARGET_LOWER_CONTEXT_BOUNDS = np.array([0.2, -0.4, -TARGET_H_W, 0.0, TARGET_TOLERANCE])
    # TARGET_UPPER_CONTEXT_BOUNDS = np.array([0.4, 0.4, 0.0, TARGET_H_W, TARGET_TOLERANCE+TARGET_TOLERANCE_W])
    # LOWER_CONTEXT_BOUNDS = np.array([-0.4, -0.4, -0.15, 0., TARGET_TOLERANCE])
    # UPPER_CONTEXT_BOUNDS = np.array([0.4, 0.4, 0., 0.15, 0.2])

    INITIAL_MEAN = np.array([0.0, 0.125, 0.5, 0.5])
    INITIAL_VARIANCE = np.diag(np.square([0.25, 0.125, 0.25, 0.25]))
    TARGET_HY = 0.0
    TARGET_HY_W = 0.1
    TARGET_TOLERANCE = 0.0
    TARGET_TOLERANCE_W = 0.1
    TARGET_LOWER_CONTEXT_BOUNDS = np.array([0.75, 0.0, TARGET_HY, TARGET_TOLERANCE])
    TARGET_UPPER_CONTEXT_BOUNDS = np.array([1.0,  1.0, TARGET_HY+TARGET_HY_W, TARGET_TOLERANCE+TARGET_TOLERANCE_W])
    LOWER_CONTEXT_BOUNDS = np.zeros(4)
    UPPER_CONTEXT_BOUNDS = np.ones(4)

    def target_log_likelihood(self, cs):
        l0, l1, l2, l3 = self.TARGET_LOWER_CONTEXT_BOUNDS
        u0, u1, u2, u3 = self.TARGET_UPPER_CONTEXT_BOUNDS
        area_target = (u0 - l0) * (u1 - l1)
        area_rest = np.prod(self.UPPER_CONTEXT_BOUNDS[:2] - self.LOWER_CONTEXT_BOUNDS[:2]) - area_target
        min_p = 1e-6 # 1e-4
        norm = (area_target * 1 + area_rest * min_p) * (
            self.TARGET_HY_W * self.TARGET_TOLERANCE_W * 1 + (
                (u2 - l2)*(u3 - l3)-(self.TARGET_HY_W * self.TARGET_TOLERANCE_W)) * min_p) 
        # in_support = (l0<=cs[:,0]<=u0) & (l1<=cs[:,1]<=u1) & (
        #     self.TARGET_HY_W+self.TARGET_HY >= cs[:, 2] >= self.TARGET_HY) & (
        #         self.TARGET_TOLERANCE_W+self.TARGET_TOLERANCE >= cs[:, 3] >= self.TARGET_TOLERANCE)
        # rewrite in_support with separate conditions
        in_support = (l0<=cs[:,0]) & (cs[:,0]<=u0) & (l1<=cs[:,1]) & (cs[:,1]<=u1) & (
            self.TARGET_HY_W+self.TARGET_HY >= cs[:, 2]) & (cs[:, 2] >= self.TARGET_HY) & (
                self.TARGET_TOLERANCE_W+self.TARGET_TOLERANCE >= cs[:, 3]) & (cs[:, 3] >= self.TARGET_TOLERANCE)
        return np.where(in_support, np.log(1 / norm) * np.ones(cs.shape[0]), np.log(min_p / norm) * np.ones(cs.shape[0]))

    def target_sampler(self, n, rng=None):
        if rng is None:
            rng = np.random
        return rng.uniform(self.TARGET_LOWER_CONTEXT_BOUNDS, self.TARGET_UPPER_CONTEXT_BOUNDS, size=(n, 4))

    DIST_TYPE = "gaussian"  # "cauchy"

    STD_LOWER_BOUND = np.array([0.05, 0.05, 0.05, 0.05])
    KL_THRESHOLD = 8000.
    KL_EPS = 0.1 # 0.25
    DELTA = 0.6
    DELTA_CS = 0.0 
    DELTA_CT = 1.0
    METRIC_EPS = 0.05 # 0.1 # 0.25 # 0.5
    INIT_CONTEXT_NUM = 100 # 200
    EP_PER_UPDATE = 40 # 20 # 10
    PEN_COEFT = 0.0
    ...
    STEPS_PER_EPOCH = 4000 # 5000 # 1000
    ...
    def create_environment(self, evaluation=False):
        env_id = "ContextualSafetyPush4D-v0"
        special_kwargs = {}
        if evaluation or self.curriculum.default():
            ...
            samples = np.random.uniform(self.LOWER_CONTEXT_BOUNDS, self.UPPER_CONTEXT_BOUNDS, 
                                        size=(1000, self.LOWER_CONTEXT_BOUNDS.shape[0]))
            ...
            bins = 10
            teacher = ACL(bins * bins * bins * bins, self.ACL_ETA, eps=self.ACL_EPS, norm_hist_len=2000)
            teacher_id = "ACL"
            ...
            special_kwargs['value_fn'] = ValueFunction(6 + self.LOWER_CONTEXT_BOUNDS.shape[0],
    ...
    def create_learner_params(self):
        algo_specific_cfgs = {
            ...
            Learner.PPO:  {
                'algo_cfgs': {
                    'steps_per_epoch': self.STEPS_PER_EPOCH, # to eval, log, actor scheduler step
                    'update_iters': 12, # 15, # 20, # 25, # 40, # 12 # gradient steps
                    'batch_size': 64, # 128
                    'target_kl': 0.02,
                    'entropy_coef': 0.0,
        ...
        custom_cfgs = {
            ...
            'model_cfgs':  {
                ...
                'actor': {
                    'hidden_sizes': [256, 256, 256], # [256, 256], # [128, 128], # [64, 64],
                    'activation': "tanh", 
                    'lr': 3e-4,
                },
                'critic': {
    ...
    def create_experiment(self):
        ...
        if str(model.agent._env._env.teacher)=="vds":
            ...
            state_provider = lambda contexts: np.concatenate(
                (ContextualSafetyPush4D.sample_initial_state(contexts.shape[0]), contexts.shape), axis=-1),
    ...
    def create_self_paced_teacher(self):
        ...
        init_lb = np.concatenate((self.LOWER_CONTEXT_BOUNDS[0:3], [self.UPPER_CONTEXT_BOUNDS[3]]))
        init_ub = np.concatenate((np.array([0.25]), self.UPPER_CONTEXT_BOUNDS[1:3], [self.UPPER_CONTEXT_BOUNDS[3]]))
        if self.curriculum.self_paced():
            ...
            init_samples = np.random.uniform(init_lb, init_ub, size=(self.INIT_CONTEXT_NUM, init_lb.shape[0]))
