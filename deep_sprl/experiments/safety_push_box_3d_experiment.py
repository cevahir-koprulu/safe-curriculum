...
from deep_sprl.environments.safety_push_box import ContextualSafetyPushBox3D

class PushBoxSampler:
    def __init__(self):
    ...
    def sample(self):
        ...
        while not ContextualSafetyPushBox3D._is_feasible(sample):
            sample = np.random.uniform(self.LOWER_CONTEXT_BOUNDS, self.UPPER_CONTEXT_BOUNDS)
        return sample
...
class SafetyPushBox3DExperiment(AbstractExperiment):
    ...
    def create_environment(self, evaluation=False):
        env_id = "ContextualSafetyPushBox3D-v0"
        special_kwargs = {}
        if evaluation or self.curriculum.default():
            teacher = PushBoxSampler()
            ...
            special_kwargs['value_fn'] = ValueFunction(97 + self.LOWER_CONTEXT_BOUNDS.shape[0],
    ...
    def create_experiment(self):
        ...
        if str(model.agent._env._env.teacher)=="vds":
            ...
            state_provider = lambda contexts: np.concatenate(
                (ContextualSafetyPushBox3D.sample_initial_state(contexts.shape[0]), contexts.shape), axis=-1),
