...
from deep_sprl.environments.safety_push import ContextualSafetyReach3D
...
class ReachSampler:
    ...
    def sample(self):
        ...
        while not ContextualSafetyReach3D._is_feasible(sample):
            sample = np.random.uniform(self.TARGET_LOWER_CONTEXT_BOUNDS, self.TARGET_UPPER_CONTEXT_BOUNDS)
        return sample
...
class SafetyReach3DExperiment(AbstractExperiment):
    ...
    def create_environment(self, evaluation=False):
        env_id = "ContextualSafetyReach3D-v0"
        special_kwargs = {}
        if evaluation or self.curriculum.default():
            teacher = ReachSampler()
    ...
    def create_learner_params(self):
        algo_specific_cfgs = {
            ...
            Learner.PPOLag:  {
                'algo_cfgs': {
                    'steps_per_epoch': self.STEPS_PER_EPOCH, # to eval, log, actor scheduler step
                    'update_iters': 15, # 15, # 12, # 25, # 40, # 12 # gradient steps
    ...
    def create_experiment(self):
        ...
        if str(model.agent._env._env.teacher)=="vds":
            ...
            state_provider = lambda contexts: np.concatenate(
                (ContextualSafetyReach3D.sample_initial_state(contexts.shape[0]), contexts.shape), axis=-1),
