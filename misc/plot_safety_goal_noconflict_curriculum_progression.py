...
from deep_sprl.experiments.safety_goal_noconflict_3d_experiment import SafetyGoalNoConflict3DExperiment
...
def plot_curriculum_progression(base_log_dir, seeds, algorithm, iterations, figname_extra):
    ...
    exp = SafetyGoalNoConflict3DExperiment(base_log_dir="logs", curriculum_name="default", 
    ...
    for algo_dict in algorithm:
        ...
        base_dir = os.path.join(base_log_dir, "safety_goal_noconflict_3d", algorithm_name, model)
        print(base_dir)
        ...
        for i, iteration in enumerate(iterations):
            ...
            f.add_axes(ax)
            # add_map(ax)
            im_frame = Image.open('/home/ck28372/safe-curriculum/safety_goal_noconflict_birdeye.png').convert('RGB')
            # save the frame
        ...
        figpath = os.path.join(Path(os.getcwd()).parent, "figures", f"safety_goal_noconflict_3d_curriculum_progressions_{label}_iter={iterations}{figname_extra}.pdf")
        print(figpath)
