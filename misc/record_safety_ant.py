...
def load_eval_contexts():
    return np.load(os.path.join(Path(os.getcwd()).parent, "eval_contexts", f"safety_ant_3d_eval_contexts.npy"))
...
def plot_trajectories(base_log_dir, policy_from_iteration, seeds, exp,
    ...
    for algo_i, algo in enumerate(algorithms):
        ...
        for seed_i, seed in enumerate(seeds):
            omnisafe_dir_file = os.path.join(base_log_dir, "logs", "safety_ant_3d", algorithm, model, f"seed-{seed}", "omnisafe_log_dir.txt")
    ...
    vid_path = os.path.join(vid_dir,
                            f"safety_ant_3d_{vidname}{vidname_extra}_c=({context[0]:.2f},{context[1]:.2f},{context[2]:.2f})"+\
                            f"_iter={policy_from_iteration}.mp4")
    print(vid_path)

    ani = animation.ArtistAnimation(fig, merged_frames, interval=100, blit=True, repeat=False)
    ani.save(filename=vid_path, dpi=200)

def main():
    base_log_dir = Path(os.getcwd()).parent
    policy_from_iteration = 95
    seeds = [str(i) for i in range(1, 4)]
    vidname_extra = "_GS=10_EPU=120_SHz"
    discount_factor = 0.99

    context = np.array([ 0., -1,  0.75])

    from deep_sprl.experiments import SafetyAnt3DExperiment
    exp = SafetyAnt3DExperiment(base_log_dir="logs", curriculum_name="default",
                                 learner_name="PPOLag", 
                                 parameters={},
                                 seed=1, device="cpu")
    algorithms = {
