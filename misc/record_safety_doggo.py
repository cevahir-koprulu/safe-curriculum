...
def load_eval_contexts():
    return np.load(os.path.join(Path(os.getcwd()).parent, "eval_contexts", f"safety_doggo_3d_eval_contexts.npy"))
...
def plot_trajectories(base_log_dir, policy_from_iteration, seeds, exp,
    ...
    for algo_i, algo in enumerate(algorithms):
        ...
        for seed_i, seed in enumerate(seeds):
            omnisafe_dir_file = os.path.join(base_log_dir, "logs", "safety_doggo_3d", algorithm, model, f"seed-{seed}", "omnisafe_log_dir.txt")
    ...
    vid_path = os.path.join(vid_dir,
                            f"safety_doggo_3d_{vidname}{vidname_extra}_c=({context[0]:.2f},{context[1]:.2f},{context[2]:.2f})"+\
...
def main():
    ...
    # seeds = ["1", "3"]
    vidname_extra = "_act=tanh_SMALLv4_h3x256"
    discount_factor = 0.99

    context = np.array([0.2,  0., 0.2])
