...
def load_eval_contexts():
    return np.load(os.path.join(Path(os.getcwd()).parent, "eval_contexts", f"safety_goal_with_vases_3d_eval_contexts.npy"))
...
def plot_trajectories(base_log_dir, policy_from_iteration, seeds, exp,
    ...
    for algo_i, algo in enumerate(algorithms):
        ...
        for seed_i, seed in enumerate(seeds):
            omnisafe_dir_file = os.path.join(base_log_dir, "logs", "safety_goal_with_vases_3d", algorithm, model, f"seed-{seed}", "omnisafe_log_dir.txt")
    ...
    vid_path = os.path.join(vid_dir,
                            f"safety_goal_with_vases_3d_{vidname}{vidname_extra}_c=({context[0]:.2f},{context[1]:.2f},{context[2]:.2f})"+\
...
def main():
    ...
    policy_from_iteration = 70
    # seeds = [str(i) for i in range(1, 4)]
    seeds = [1]
    vidname_extra = "_DCS=0.0_newinitv2_nn256"
    discount_factor = 0.99

    context = np.array([0., 1, 0.5])

    from deep_sprl.experiments import SafetyGoalWithVases3DExperiment
    exp = SafetyGoalWithVases3DExperiment(base_log_dir="logs", curriculum_name="default",
                                 learner_name="PPOLag", 
                                 parameters={},
                                 seed=1, device="cpu")
    algorithms = {
