...
def load_eval_contexts():
    return np.load(os.path.join(Path(os.getcwd()).parent, "eval_contexts", f"safety_push_4d_eval_contexts.npy"))
...
def plot_trajectories(base_log_dir, policy_from_iteration, seeds, exp,
    ...
    for algo_i, algo in enumerate(algorithms):
        ...
        for seed_i, seed in enumerate(seeds):
            omnisafe_dir_file = os.path.join(base_log_dir, "logs", "safety_push_4d", algorithm, model, f"seed-{seed}", "omnisafe_log_dir.txt")
            ...
            if policy is None:
                print(f"Policy not found for {algo} and seed {seed}")
                continue
    ...
    vid_path = os.path.join(vid_dir,
                            f"safety_push_4d_{vidname}{vidname_extra}_c=({context[0]:.2f},{context[1]:.2f},{context[2]:.2f},{context[3]:.2f})"+\
...
def main():
    ...
    policy_from_iteration = 72
    seeds = [str(i) for i in range(1, 4)]
    vidname_extra = "_DCS=0.0_MEPS=0.05"
    discount_factor = 0.99

    context = np.array([0.3, 0.75, 0.25, 0.6])

    from deep_sprl.experiments import SafetyPush4DExperiment
    exp = SafetyPush4DExperiment(base_log_dir="logs", curriculum_name="default",
                                 learner_name="PPOLag", 
                                 parameters={},
                                 seed=1, device="cpu")
    algorithms = {
        "SCG": {
            ...
            "label": "SCG",
            "model": "PPOLag_DELTA_CS=0.0_ATP=1.0_CAS=10_DELTA=0.6_DELTA_CT=1.0_METRIC_EPS=0.05_RAS=10",
            "color": "red",
        },
        "CRT": {
            "algorithm": "wasserstein",
            "label": "CRT",
            "model": "PPOLag_DELTA_CS=0.0_DELTA=0.6_METRIC_EPS=0.05_PEN_COEFT=0.0",
            "color": "blue",
        },  
        # "DEF": {
        #     "algorithm": "default",
